...
**待團主決定：**
- `59282a9` 把「未開團不顯示結單時間」改回一律顯示，列表也一樣。但後端 `set_campaign_status` 提前結單時不會清掉 `auto_close_at`，所以提前結單的團會同時顯示「已結單」和一個還沒到的結單時間。另外，`App.test.tsx` 的測試名稱仍寫 hides the closing time，內容卻斷言時間有顯示，兩者不一致。
