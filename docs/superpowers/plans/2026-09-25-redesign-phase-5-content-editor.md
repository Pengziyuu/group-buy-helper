...
- **執行中對計畫的修正**：
  ...
  - **最終審查修正（`8044beb`）：**新品項預設名稱空白，但正式資料庫的 `valid_campaign_items` 會拒絕名稱空白的品項。原本自動儲存會失敗，而且在該列取名前其他修改也存不進去。改為任一品項名稱空白時暫停自動儲存，並顯示「有欄位需要修正，修正後才會自動儲存」。另外修正本機示範在儲存丟出例外時卡在「儲存中…」的問題。修正後共 697 個測試全數通過。
- **延後的小問題**：
  ...
  - 最終審查的其他小問題（未修）：
    - 公告空白時，「公告與圖片」標為未完成，但發布前檢查不要求公告，兩者不一致。這是計畫的設計，是否調整由團主決定。
    - 圖片載入失敗時，預覽裡會多一個 `role="status"`。
    - 圖片錯誤清單的 `role="alert"` 放在 `<ul>` 上，會失去清單語意。
    - 用滑鼠點「加入圖片」也會出現聚焦外框。
    - 本機示範一律帶住戶頁連結。
- **需要團主在正式環境確認的事項**：
