...
describe('OrganizerHome', () => {
  ...
  it('shows status, progress, orders, closing time and time for each phase without payment columns', () => {
    ...
    expect(within(openRow).getByRole('rowheader', { name: '冰餅團' })).toBeInTheDocument()
    expect(openRow.querySelector('img.organizer-thumb')).toHaveAttribute('src', 'https://example.com/ice.jpg')
    ...
    expect(draftRow.querySelector('.organizer-thumb-empty')).toBeInTheDocument()
