...
export function createPickupNotificationHandler(destination: 'production' | 'test') {
  ...
  try {
    ...
    const { data: testMarker, error: testMarkerError } = await service
      .from('pickup_notification_test_campaign')
      .select('campaign_id')
      .eq('campaign_id', campaignId)
      .maybeSingle()
    if (testMarkerError) throw testMarkerError
    const isTestCampaign = Boolean(testMarker?.campaign_id)
    if ((destination === 'test') !== isTestCampaign) {
      return jsonResponse({ error: destination === 'test' ? '此團購尚未加入通知測試中心' : '測試團購不能從正式介面發送' }, 409)
    }
