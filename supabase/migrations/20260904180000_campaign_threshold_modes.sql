-- Let each campaign use its existing formation threshold as either a quantity
-- target or a total-amount target. Existing rows remain quantity campaigns.

alter table public.campaign
  add column threshold_kind text not null default 'quantity'
  check (threshold_kind in ('quantity', 'amount'));
alter table public.campaign
  add column amount_threshold numeric(14,2);
alter table public.campaign
  add constraint campaign_threshold_configuration_valid check (
    (threshold_kind = 'quantity' and amount_threshold is null)
    or
    (threshold_kind = 'amount' and amount_threshold > 0
      and amount_threshold <= 999999999999.99
      and amount_threshold = round(amount_threshold, 2))
  );
...
alter table public.campaign_draft
  add column amount_threshold numeric(14,2);
alter table public.campaign_draft
  add constraint campaign_draft_threshold_configuration_valid check (
    (threshold_kind = 'quantity' and amount_threshold is null)
    or
    (threshold_kind = 'amount' and amount_threshold > 0
      and amount_threshold <= 999999999999.99
...
-- Preserve historical orders while normalizing the new quantity invariant.
-- The preceding ALTER TABLE locks serialize this migration with order writes.
update public.campaign campaign
set status = 'closed'
where campaign.status = 'open'
  and campaign.threshold_kind = 'quantity'
  and (
...
begin
  ...
  if v_draft.threshold_kind = 'quantity' and v_current_quantity > v_draft.threshold then
    raise exception '目前已有 % 個，數量門檻不能低於目前訂購數量', v_current_quantity
      using errcode = '23514';
  end if;
...
begin
  ...
  if v_campaign.threshold_kind = 'quantity' then
    ...
    if v_other_quantity + v_desired_quantity > v_campaign.threshold then
      raise exception '此訂單最多可保留 % 個，請減少 % 個',
        v_max_order_quantity, v_desired_quantity - v_max_order_quantity
        using errcode = '23514';
    end if;
  end if;
...
begin
  ...
  if p_status = 'open' and v_campaign.threshold_kind = 'quantity' then
    ...
    if v_current_quantity >= v_campaign.threshold then
      raise exception '目前數量已達成團門檻，不能重新開團' using errcode = '23514';
    end if;
  end if;
...
select id, slug, title, unit_price, threshold, status, deadline, announcement,
       images, items, opened_at, created_at, updated_at, threshold_kind, amount_threshold
from public.campaign;
...
as $$
  select campaign.slug, campaign.title, campaign.unit_price, campaign.threshold,
         ...
         campaign.threshold_kind, campaign.amount_threshold,
