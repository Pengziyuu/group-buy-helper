-- Promote the authoritative formation target to organizer campaign cards.
-- Keep all prior return columns for deployment compatibility with the existing client.
...
begin
  ...
  select campaign.id,
         ...
         case when campaign.opened_at is null
           then coalesce(draft.threshold_kind, campaign.threshold_kind)
           else campaign.threshold_kind
         end as threshold_kind,
         case when campaign.opened_at is null
           then coalesce(draft.threshold, campaign.threshold)
           else campaign.threshold
         end as threshold,
         case when campaign.opened_at is null
           then draft.amount_threshold
           else campaign.amount_threshold
